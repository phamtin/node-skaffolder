...
const generateAuthTokens = async (user) => {
  ...
  const accessTokenExpires = addMinutes(
    new Date(),
    jsonwebtoken.accessExpirationMinutes
  ); // 1 minutes
  const accessToken = signToken(_id, accessTokenExpires, "access");
  const refreshTokenExpires = addHours(
    new Date(),
...
const generateResetPasswordToken = async (email) => {
  ...
  const expires = addMinutes(
    new Date(),
    jsonwebtoken.resetPasswordExpirationMinutes
  ); // 5 minutes
...
const generateVerifyEmailToken = async (user) => {
  const expires = addMinutes(
    new Date(),
    jsonwebtoken.verifyEmailExpirationMinutes
  ); // 5 minutes
